...
type PlanContextType = {
  ...
  setDailyOverrides: React.Dispatch<
...
export const PlansProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    if (!isLoading) {
      AsyncStorage.setItem("weeklySchedule", JSON.stringify(schedule)).catch(
        (err) => console.error("Fehler beim Speichern des Schedules:", err),
      );
    }
  }, [schedule, isLoading]);
  ...
  useEffect(() => {
    if (!isLoading) {
      AsyncStorage.setItem("workoutLog", JSON.stringify(workoutLog)).catch(
        (err) => console.error("Fehler beim Speichern des Workout Logs:", err),
      );
    }
  ...
  const updateExerciseData = (
    ...
    setPlans((prevPlans) => {
      ...
      AsyncStorage.setItem("gymPlans", JSON.stringify(updatedPlans)).catch(
        (err) =>
          console.error(
            "Fehler beim direkten Speichern in updateExerciseData:",
            err,
          ),
      );
